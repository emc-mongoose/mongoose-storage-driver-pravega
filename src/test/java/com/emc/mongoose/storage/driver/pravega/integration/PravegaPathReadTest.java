...
public class PravegaPathReadTest {
  ...
  @Test
  public void testPathRead() throws Exception {
    ...
    try (final ClientFactory clientFactory = ClientFactory.withScope(scope, controllerURI);
        EventStreamWriter<String> writer =
            clientFactory.createEventWriter(
                streamName, new JavaSerializer<String>(), EventWriterConfig.builder().build())) {
    ...
    try (final ReaderGroupManager readerGroupManager =
    ...
    try (final ClientFactory clientFactory = ClientFactory.withScope(scope, controllerURI);
      ...
      if (event.getEvent() != null) {
        ...
        assertEquals(
            "we didn't read the event string we had put into stream", "message1", event.getEvent());
      }
      event = reader.readNextEvent(readerTimeoutMs);
      if (event.getEvent() != null) {
        System.out.format("Read event '%s'%n", event.getEvent());
        assertEquals(
            "we didn't read the event string we had put into stream", "message2", event.getEvent());
      }
      event = reader.readNextEvent(readerTimeoutMs);
      if (event.getEvent() != null) {
        System.out.format("Read event '%s'%n", event.getEvent());
        assertEquals(
            "we didn't read the event string we had put into stream", "message3", event.getEvent());
      }
      event = reader.readNextEvent(readerTimeoutMs);
