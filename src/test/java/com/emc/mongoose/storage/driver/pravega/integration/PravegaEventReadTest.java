...
public class PravegaEventReadTest {
  ...
  public void testEventRead() throws Exception {
    ...
    val streamConfig = StreamConfiguration.builder().scalingPolicy(ScalingPolicy.fixed(1)).build();
    streamManager.createStream(scopeName, streamName, streamConfig);
    try (val clientFactory = ClientFactory.withScope(scopeName, controllerURI);
        val writer =
    ...
    try (val clientFactory = ClientFactory.withScope(scopeName, controllerURI);
      ...
      if (event1.getEvent() != null) {
        ...
        assertEquals(
            "we didn't read the event string we had put into stream",
            "TestEvent",
            event1.getEvent());
      }
