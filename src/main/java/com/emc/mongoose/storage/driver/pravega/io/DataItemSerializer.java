...
public final class DataItemSerializer implements Serializer<DataItem>, Serializable {
  ...
  /**
   * @param useDirectMem Specifies whether should it use direct memory for the resulting buffer
   *     containing the serialized data or not. Using the direct memory may lead to the better
   *     performance in case of large data chunks but less safe.
   */
   ...
   * @throws OutOfMemoryError if useDirectMem is disabled and not enough memory to allocate the
   *     buffer
   */
  @Override
    ...
    try {
      ...
      if (MAX_EVENT_SIZE < dataItemSize) {
        Loggers.ERR.warn(
            ...
            SizeInBytes.formatFixedSize(dataItemSize),
            SizeInBytes.formatFixedSize(MAX_EVENT_SIZE));
      }
      final var dstBuff =
          useDirectMem
              ? ByteBuffer.allocateDirect((int) dataItemSize)
              : // will crash if not enough memory
